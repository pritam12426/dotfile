...
# Source https://github.com/peterkaminski/obsidian-settings-manager
...
def init_argparse():
    ...
    parser = argparse.ArgumentParser(
        description="Manage Obsidian settings across multiple vaults."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Print what the file system operations are happening",
    )
    parser.add_argument(
        "--dry-run",
        "-n",
        action="store_true",
        help="Do a dry-run. Show what would be done, without doing it.",
    )
    parser.add_argument(
        "--root",
        default=OBSIDIAN_ROOT_DIR,
        help=f"Use an alternative Obsidian Root Directory (default {OBSIDIAN_ROOT_DIR!r})",
    )
    only_one_of = parser.add_mutually_exclusive_group(required=True)
    only_one_of.add_argument(
        "--list", "-l", action="store_true", help="list Obsidian vaults"
    )
    only_one_of.add_argument(
        "--update", "-u", help="update Obsidian vaults from UPDATE vault"
    )
    only_one_of.add_argument(
        "--exact-copy-of",
        help="delete and recreate Obsidian vaults with an exact copy of the EXACT_COPY_OF vault",
    )
    only_one_of.add_argument(
        "--diff-to",
        "-d",
        help="Like update but instead of copying, just show a diff against DIFF_TO instead (no changes made).",
    )
    only_one_of.add_argument(
        "--execute", "-x", help="run EXECUTE command within each vault (use caution!)"
    )
    only_one_of.add_argument(
        "--backup-list",
        action="store_true",
        help="list ISO 8601-formatted .obsidian backup files from all vaults",
    )
    only_one_of.add_argument(
        "--backup-remove",
        action="store_true",
        help="remove ISO 8601-formatted .obsidian backup files from all vaults",
    )
    only_one_of.add_argument(
...
def main():
    ...
    try:
        ...
        if args.version:
            ...
            call_for_each_vault(
                vault_paths, copy_settings, Path.home() / args.update, clean_first=False
            )
