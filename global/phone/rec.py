...
def main():
    ...
    parser.add_argument("-c", "--camera", choices=['back', 'front'],
                       help="Record camera directly (non-interactive)")

    # Additional options
    parser.add_argument("-nd", "--no-display", action="store_true",
                       help="No display (headless)")
    parser.add_argument("-a", "--audio", choices=['mic', 'system', 'none'],
    ...
    if args.record:
        ...
        if source == 'screen':
            ...
            if camera_side == 'back':
                COMMAND.extend(["--video-source=camera", "--camera-facing=back",
                              "-r", f"{file_path}_back-cam.mp4"])
            else:
                COMMAND.extend(["--video-source=camera", "--camera-facing=front",
                              "-r", f"{file_path}_front-cam.mp4"])
