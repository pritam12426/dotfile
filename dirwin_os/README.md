## References 

- [Github awesome-tuis](https://github.com/rothgar/awesome-tuis/)
- [Terminaltrove](https://terminaltrove.com)
- [Suckless](https://suckless.org/rocks/)
- [Asciinema](https://asciinema.org/)
- [FreeCodeCamp](https://www.freecodecamp.org/news/essential-cli-tui-tools-for-developers/)

## Install with Bin command

- [adbtuifm](https://github.com/darkhz/adbtuifm) (Regularly Used)
- [aichat](https://terminaltrove.com/aichat/) (Not Regularly Used)
- [arduino-cli](https://github.com/arduino/arduino-cli)  (Regularly Used)
- [asciinema](https://github.com/asciinema/asciinema)  (Regularly Used)
- [atac](https://github.com/Julien-cpsn/ATAC)  (Not Regularly Used)
- [bat](https://github.com/charlie0129/batt/releases)  (Regularly Used)
- [batt](https://github.com/charlie0129/batt)  (Regularly Used)
- [bin](https://github.com/marcosnils/bin) (Regularly Used)
- [broot](https://github.com/Canop/broot)  (Regularly Used)
- [caligula](https://terminaltrove.com/caligula/)  (Not Regularly Used)
- [cargo-seek](https://github.com/tareqimbasher/cargo-seek) (Regularly Used)
- [croc](https://github.com/schollz/croc) (Regularly Used)
- [d2](https://github.com/terrastruct/d2) (Regularly Used)
- [doggo](https://github.com/mr-karan/doggo)  (Regularly Used)
- [dog](https://github.com/ogham/dog) with RUST (Regularly Used)
- [dra](https://terminaltrove.com/dra/) (Not Regularly Used)
- [dua](https://github.com/Byron/dua-cli) (Regularly Used)
- [dua](https://terminaltrove.com/dua/) (Regularly Used)
- [duf](https://github.com/muesli/duf) (Regularly Used)
- [dust](https://github.com/bootandy/dust) (Regularly Used)
- [fd](https://github.com/sharkdp/fd) (Regularly Used)
- [fzf](https://github.com/junegunn/fzf) (Regularly Used)
- [gh](https://github.com/cli/cli) (Regularly Used)
- [hurl](https://hurl.dev/) (Regularly Used)
- [joshuto - ranger](https://github.com/kamiyaa/joshuto) (Regularly Used)
- [lazygit](https://github.com/jesseduffield/lazygit) (Regularly Used)
- [lf](https://github.com/gokcehan/lf) (Regularly Used)
- [neocmakelsp-v0_8.19](https://github.com/neocmakelsp/neocmakelsp) (Regularly Used)
- [pinentry-touchid](https://github.com/jorgelbg/pinentry-touchid) (Regularly Used)
- [rclone](https://github.com/rclone/rclone)  (Regularly Used)
- [rg](https://github.com/BurntSushi/ripgrep)  (Regularly Used)
- [ruff](https://github.com/astral-sh/ruff)  (Regularly Used)
- [scrcpy](https://github.com/Genymobile/scrcpy) (Regularly Used)
- [shellcheck](https://github.com/koalaman/shellcheck)  (Regularly Used)
- [shfmt](https://github.com/patrickvane/shfmt) (Regularly Used)
- [shiori](https://github.com/go-shiori/shiori) (Regularly Used)
- [sk](https://github.com/skim-rs/skim) `A fuzzy finder written in Rust.` (Regularly Used)
- [syncthing](https://syncthing.net)(Regularly Used)
- [taplo](https://github.com/tamasfe/taplo) (Regularly Used)
- [tre](https://github.com/dduan/tre) (Regularly Used)
- [wireproxy](https://www.youtube.com/watch?v=ESr0xid-kl4) (Regularly Used)
- [zellij](https://github.com/zellij-org/zellij) (Not Regularly Used)
- [zenity](https://github.com/ncruces/zenity) (Regularly Used)

## Manual / Releases

- [gnu stow](https://www.gnu.org/software/stow/) `install manually with   https://formulae.brew.sh/formula/stow ` (Not Regularly Used)
- [mac-cleanup](https://github.com/mac-cleanup) `manually install this for the  Releases page` (Regularly Used)
- [pandoc](https://github.com/jgm/pandoc) `manually install this for the  Releases page` (Regularly Used)
- [qmv](https://github.com/itchyny/mmv) `manually install this for the  Releases page` (Not Regularly Used)
- [shadowsocks](https://github.com/shadowsocks) `manually install this for the  Releases page` (Not Regularly Used)
- [sleek](https://github.com/nrempel/sleek) `sql formater manually install this for the  Releases page` (Regularly Used)
- [sqlmap]() `manually install this for the  Releases page` (Not Regularly Used)
- [vidir](https://github.com/trapd00r/vidir/blob/master/bin/vidir) `manually install this for the  Releases page` (Not Regularly Used)
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) Media downlader. `manually install this for the  Releases page` (Regularly Used)

## Github / Binary / Build From Source

- [clock](https://www.youtube.com/watch?v=0rUBhqR6ckw&t=12242s) `You have the binary on you github` (Regularly Used)
- [cointop](https://github.com/cointop-sh/cointop) `You have the binary on you github` (Regularly Used)
- [aria2c](https://github.com/aria2/aria2) `You have the binary on you github` (Regularly Used)
- [fclones](https://terminaltrove.com/fclones/) duplicate files on your system `you have the binary on you github` (Regularly Used)
- [tig](https://github.com/jonas/tig) `You have the binary on you github / build from source for new verison esay to install` (Regularly Used)
- [entr](https://github.com/eradman/entr) `You have the binary on you github / build from source for new verison esay to install` (Regularly Used)
- [pstree](https://github.com/FredHucht/pstree) `You have the binary on you github` (Regularly Used)
- [htop]() `You have the binary on you github` (Regularly Used)
- [tree]() `you have the binary on you github` (Regularly Used)
- [envpath]() `You have the binary on you github` (Regularly Used)
- [gsed]() `You have the binary on you github` (Regularly Used)
- [ncdu]() `You have the binary on you github` (Regularly Used)
- [pkg-config]() `You have the binary on you github` (Not Regularly Used)
- [pkgconf]() `You have the binary on you github` (Regularly Used)

## Ships With Command Line Tools

- [clang-stat-cache]() `Ships with command online tools  /Library/Developer/CommandLineTools/usr/bin` (Not Regularly Used)
- [clang-tidy]() `Ships with command online tools  /Library/Developer/CommandLineTools/usr/bin` (Not Regularly Used)

## Other / Unspecified

- [ADB](https://github.com/Genymobile/scrcpy) tool for working with add `Static binary is there of the googel adb page or go with scrcpy` (Regularly Used)
- [axel](https://github.com/axel-download-accelerator/axel) (Not Regularly Used)
- [sntop](https://terminaltrove.com/sntop/) (Not Regularly Used)
- [goaccess](https://terminaltrove.com/goaccess/) (Not Regularly Used)
- [fastfetch / neofeatch]() (Not Regularly Used)
- [doxygen]() (Not Regularly Used)
- [gradle]() (Regularly Used)
- [jdtls]() (Not Regularly Used)
- [ninja]() (Regularly Used)
- [osm]() (Regularly Used)
- [rr]() (Regularly Used)
- [rrr]() (Regularly Used)
- [rrrr]() (Not Regularly Used)
- [sql-fmt]() (Not Regularly Used)



## Temp






Here’s a curated list of popular self-hosted CLI (or daemon + CLI/web UI) tools that people who love Syncthing, rclone, and aria2 tend to really enjoy. They’re all open-source, run on your own hardware (Linux, Raspberry Pi, NAS, VPS, etc.), and give you full control over your data.

### File Sync / Backup / Transfer
- **restic** – Fast, efficient, deduplicated, encrypted backups (like rclone serve + backup)
- **rclone** – (you already know) Swiss army knife for cloud storage
- **duplicati** – Backup tool with GUI but also strong CLI, encryption, deduplication
- **borgbackup** / **borgmatic** – Deduplicated, encrypted, compressed backups (very popular in the self-hosted community)
- **rsync** + **rsync daemon** – The classic, still unbeatable for local/LAN sync
- **seafile-cli** (Seafile sync client) – Dropbox-like sync but fully self-hosted
- **nextcloudcmd** (Nextcloud CLI client) – For syncing with your Nextcloud instance without the full desktop client
- **zrepl** – ZFS replication tool (if you’re into ZFS)

### Download Managers (like aria2)
- **yt-dlp** – The ultimate YouTube + 10,000 sites downloader (successor to youtube-dl)
- **motrix** (has CLI mode) – But most people use:
- **jdownloader2** (headless mode) – Powerful for hosters (rapidgator, uploaded, etc.)
- **persepolis** – GUI for aria2, but you can just use aria2c directly
- **axel** – Lightweight multi-threaded downloader
- **lftp** – Extremely powerful FTP/HTTP/SFTP client with mirroring, queuing, etc.

### File Sharing / Remote Access
- **croc** – Simple, secure, p2p file transfer (like magic-wormhole but easier)
- **magic-wormhole** – Send files point-to-point with a code
- **transfer.sh** (self-hosted version) or **pingvin-share**, **gossa** – Tiny file servers
- **filebrowser** – Web-based file manager with CLI-like feel
- **miniserve** – One-line static file server with upload support

### Media / Torrent / Automation
- **qbittorrent-nox** – Headless qBittorrent (web UI + excellent CLI control via qbittorrent-api)
- **transmission-daemon** – Lightweight torrent client with remote CLI/web control
- **rtorrent** + **ruTorrent** (web UI) – Classic CLI torrent client loved by power users
- **sonarr**, **radarr**, **lidarr**, **readarr**, **prowlarr** – The *arr suite (automation for TV, movies, music, books)
- **bazarr** – Subtitle management
- **jellyfin** / **plex** (with CLI tools) – Media servers

### Storage & Filesystem Tools
- **mergerfs** + **snapraid** – Poor-man’s ZFS/Unraid setup
- **btrfs** + **btrbk** or **snapper** – Snapshot-based backups
- **vdo** + **lvmcache** – Deduplication/compression at block level
- **zfs** (OpenZFS) – The king of advanced filesystems

### Networking / Tunneling
- **tailscale** / **headscale** (self-hosted control server) – Zero-config VPN
- **wireguard** – Fast, modern VPN
- **frp** / **ngrok** (self-hosted alternatives: **inlets**, **ztncui**, **wstunnel**)
- **cloudflared** (Argo Tunnel) – Expose services without opening ports

### Miscellaneous Gems
- **healthchecks.io** (self-hosted version: **Healthchecks**) – Monitor your cron jobs / backups
- **changedetection.io** – Watch webpages for changes
- **uptime-kuma** – Monitor your services
- **watchtower** – Auto-update your Docker containers
- **cronicle** – Better cron replacement with web UI + CLI
- **mosh** – Better SSH for mobile connections
- **tmux** / **zellij** – Terminal multiplexers (you probably already use tmux)
- **fd**, **ripgrep (rg)**, **dust**, **duf**, **btop**, **gping** – Modern CLI replacements for find, grep, du, df, top, ping

If you want the "Syncthing + rclone + aria2 vibe" specifically (decentralized, efficient, no vendor lock-in), the absolute favorites in the community right now are:

- restic or borgbackup
- qbittorrent-nox or rtorrent
- yt-dlp
- tailscale/headscale
- the entire *arr stack
